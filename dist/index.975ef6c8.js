...
// }
let name = "";
(0, _fetchCountries.refs).input.addEventListener("input", (0, _lodashDebounceDefault.default)(readInputValue, DEBOUNCE_DELAY));
function readInputValue() {
    // if(name === ""){return}
    name = (0, _fetchCountries.refs).input.value.trim();
    (0, _fetchCountries.fetchCountries)(name, createMarkup);
}
function createMarkup(data) {
    (0, _fetchCountries.refs).list.innerHTML = "";
    (0, _fetchCountries.refs).div.innerHTML = "";
    if (data.length >= 10) return (0, _notiflixDefault.default).Notify.info("Too many matches found. Please enter a more specific name.");
    if (data.length < 10 || data.length > 1) {
        data.map((country)=>{
            ...
            (0, _fetchCountries.refs).list.insertAdjacentHTML("afterbegin", markup);
        });
        if (data.length === 1) data.map((country)=>{
            let markup = `
        <ul class= "country__info">
          <li><h3>Capital:</h3> ${country.capital}</li>
          <li><h3>Population:</h3> ${country.population} people</li>
          <li><h3>Languages:</h3> ${country.languages.map((lang)=>{
                return ` ${lang.name}`;
            })}</li>
        </ul>`;
            (0, _fetchCountries.refs).div.insertAdjacentHTML("afterbegin", markup);
        });
    }
}

},{"./css/styles.css":"1CY4s","lodash.debounce":"3JP5n","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","notiflix":"5WWYd","./fetchCountries":"fTQqv"}],"1CY4s":[function() {},{}],"3JP5n":[function(require,module,exports) {
var global = arguments[3];
/**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","notiflix":"5WWYd"}]},["1RB6v","8lqZg"], "8lqZg", "parcelRequired7c6")
